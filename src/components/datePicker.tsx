...
const DatePickerComponent: React.FC = () => {
  ...
  const DateCustomInput: React.FC<{ value: string; onClick: () => void }> = ({ value, onClick }) => (
    <input // input 요소로 변경
  ...
  const TimeCustomInput: React.FC<{ value: string; onClick: () => void }> = ({ value, onClick }) => (
    <input // input 요소로 변경
  ...
  return (
    <div className="grid grid-cols-2 gap-8 pb-6">
      ...
      <DatePicker
        ...
        dateFormat="h:mm aa"
        withPortal
        minTime={pickupDate && pickupDate.toDateString() === new Date().toDateString() ? nowMinTime : minTime}
        maxTime={maxTime}
        customInput={
          <TimeCustomInput
            value={pickupDate ? pickupDate.toLocaleDateString('ko-KR') : '날짜 선택하기'}
